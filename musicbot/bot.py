...
class MusicBot(discord.Client):
    ...
    async def _initialize_document(self, guild, id):
        users = []
    ...
    async def on_player_play(self, player, entry):
        ...
        if newmsg:
            if self.config.dm_nowplaying and author:
                if thumbnail:
                    await self.safe_send_file(author, newmsg, thumbnail)
                else:
            ...
            if thumbnail:
                self.server_specific_data[guild]['last_np_msg'] = await self.safe_send_file(channel, newmsg, thumbnail)
            else:
...
################################################################################### safe methods + final ready execution
    ...
    async def safe_send_file(self, dest, content, fp, *, tts=False, expire_in=0, also_delete=None, quiet=False, filename=None):
        msg = None
        try:
            msg = await dest.send(file = discord.File(fp), content=content, tts=tts)
        ...
        '''async def background_gogCheck(self):
            ...
            while not self.is_closed():
                ...
                if document['announcementchannel']:
                    ...
                    #try:
                    gogResults = await http_request(gogURL, "?devpub=alice_in_dissonance&mediaType=game")
                    gogResults = gogResults.get('products')
                    time = 86400
...
####################################################### custom commands
    ...
    async def cmd_purgedb(self, guild):
        ...
        """
        await self.dbservers.update_one({"server_id": guild.id, 'ruleschannel': {"$exists": false}}, {"$set": {'ruleschannel': None}})
        await self.dbservers.update_one({"server_id": guild.id, 'welcomechannel': {"$exists": false}}, {"$set": {'welcomechannel': None}})
    ...
    async def cmd_selfrole(self, guild, channel_mentions, leftover_args):
        """
        Usage:
            {command_prefix}selfrole channel_mention ["title" "role_name, emoji" "role_name, emoji" "role_name, emoji"] ["title" "role_name, emoji"]
            Creates a set of embeds that separate self-assignable roles into categories. Each [] consititutes one category. First value is the title,
            followed by role/emoji pairs.
            Ex: %selfrole #test ["Utility Roles" "Apple, :apple:" "Pizza, :pizza:"] ["Colors" "green, :green_square:" "red, :red_square:"]
        """
    ...
    async def cmd_configselfrole(self, guild, leftover_args):
        """
        Usage: 
            {commandprefix}configselfrole [edit/remove/disable] <msg_id> <optional parameters>
            Allows you to edit or remove categories from an existing self-assignable role setup, or disable it completely for the server.
            Edit accepts an optional parameter to edit one category, refer to selfrole for the proper formatting.
            Disable does not require a message id.
        """
            ...
            if action == "disable":
                async for post in self.dbselfrole.find({"guild": guild.id}):
                    ...
                    try:
                        ...
                        await self.dbselfrole.delete_many({"guild": guild.id})
                    except discord.Forbidden:
    ...
    async def cmd_stats(self, channel, player):
        ...
        content.add_field(name="Servers", value="I am running on " + str(len(self.guilds)) + " servers")
...
############################################### extra music queue management commands
    ...
    async def cmd_sub(self, player, channel, author, permissions, leftover_args, song_url, pos=None):
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            # If it's an entry
                ...
                reply_text = self.str.get('cmd-play-sub', "Substituted **%s** with **%s** at position %s")
    ...
    async def cmd_playnow(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            # If it's playlist
            if 'entries' in info:
                raise exceptions.CommandError("You cannot bypass queue with a playlist!", expire_in=30)
...
################################################ default music commands
    ...
    async def cmd_np(self, player, channel, guild, message):
        ...
        if player.current_entry:
            ...
            if thumbnail:
                self.server_specific_data[guild]['last_np_msg'] = await self.safe_send_file(channel, np_text, thumbnail)
            else:
...
############################################# message handler & command parser
    ...
    async def on_message(self, message):
        ...
        if self.config.bound_channels and message.channel.id not in self.config.bound_channels and not isinstance(message.channel, discord.abc.PrivateChannel):
            if command in self.config.bound_commands:
                return
